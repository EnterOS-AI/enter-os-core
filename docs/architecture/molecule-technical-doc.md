# EnterOS — Comprehensive Technical Documentation

> Definitive technical reference for the EnterOS Agent Team platform.
> Based on a full non-invasive scan of the [enter-os-core](https://github.com/EnterOS-AI/enter-os-core) repository.
...
EnterOS is an **org-native control plane for heterogeneous AI agent teams**. It is not a workflow builder or a replacement for agent frameworks; rather, it is the operational and organizational layer that sits above multiple runtime frameworks and provides:
...
**One-liner**: *"EnterOS is the org-native control plane for heterogeneous AI agent teams."*
...
| Category | Examples | How EnterOS Differs |
|----------|---------|---------------------|
| Workflow builders | n8n, Windmill, Temporal | EnterOS models **roles**, not tasks |
| Agent frameworks | LangGraph, CrewAI, AutoGen | EnterOS sits **above** frameworks as runtime adapters |
| Coding agents | Claude Code, Cursor, Codex | EnterOS runs coding agents as **workspace roles** alongside other types |
| Chat UIs | ChatGPT, Claude.ai | EnterOS is **operational infrastructure**, not a conversation interface |

---

## 3. System Architecture

> **Workspace placement contract:** every EnterOS org runs as a fully isolated tenant on its own EC2, with workspace-server, memory plugin, Postgres, and Redis all co-located. The platform (controlplane on Railway) handles provisioning, billing, and DNS only — it never holds tenant data. See [`workspace-placement.md`](workspace-placement.md) for the formal RFC.
...
| Docker Network | `enter-os-core-net` | Internal-only by default, no exposed DB/Redis ports |
...
- All containers join `enter-os-core-net` private network
...
EnterOS's workspace abstraction is **runtime-agnostic by design**. A workspace is a role with an A2A interface — not an LLM with a prompt. The same hierarchy, memory boundaries, approval chains, and governance that organize AI agents in containers today can organize any autonomous system that speaks A2A.
...
- **GitHub**: https://github.com/EnterOS-AI/enter-os-core
- **Architecture Docs**: https://github.com/EnterOS-AI/enter-os-core/src/branch/main/docs/architecture
- **API Protocol**: https://github.com/EnterOS-AI/enter-os-core/src/branch/main/docs/api-protocol
- **Agent Runtime**: https://github.com/EnterOS-AI/enter-os-core/src/branch/main/docs/agent-runtime
- **Product Docs**: https://github.com/EnterOS-AI/enter-os-core/src/branch/main/docs/product

---

*© 2026 EnterOS Technologies, Inc.*
