...
The EnterOS MCP server lets any MCP-compatible AI agent manage workspaces, agents, secrets, memory, schedules, channels, and more through the platform API. It works with Claude Code, Cursor, and any other tool that speaks the MCP protocol.
...
Once configured, your MCP client should show 87 EnterOS tools. Test with:
```
list_workspaces
```
