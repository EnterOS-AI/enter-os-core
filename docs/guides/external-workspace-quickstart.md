...
Run an agent on your laptop, a home server, a cloud VM, or any machine with internet — and have it show up on a EnterOS canvas alongside platform-provisioned agents. This guide gets you from zero to a working agent in under 5 minutes.
...
A workspace whose agent code lives outside EnterOS's infrastructure. The platform treats it as a first-class participant — canvas node, A2A routing, delegation, memory, channels — but doesn't manage its lifecycle (no Docker, no EC2 launched for you).
...
| You need | Notes |
|---|---|
| A EnterOS tenant | Your own hosted instance (e.g. `you.moleculesai.app`) or self-hosted |
...
1. Open your EnterOS canvas at `https://<TENANT>`
...
🎉 **You have an external agent running on EnterOS.** Everything from here is iteration on that agent's handler code.
...
| Agent duplicated in chat | Known canvas bug where WebSocket + HTTP responses both render. Fixed in [PR #1517](https://github.com/EnterOS-AI/enter-os-core/pull/1517). |
...
See the [design doc](https://git.moleculesai.app/molecule-ai/internal/src/branch/main/product/external-workspaces-polling.md) (internal) and the implementation tracking issue (search `polling+mode` on the [enter-os-core issue tracker](https://github.com/EnterOS-AI/enter-os-core/issues)).
...
- Open an issue: https://github.com/EnterOS-AI/enter-os-core/issues/new?labels=external-workspace
