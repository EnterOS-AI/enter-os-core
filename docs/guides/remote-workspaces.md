...
> your EnterOS org, appear on the canvas, receive A2A tasks from parent
...
control plane. EnterOS supported external agents as a development escape
...
| Requirement | Details |
|---|---|
| **Platform** | EnterOS platform running v0.30+ (`go run ./cmd/server` from `workspace-server/` or the current `main` image) |
...
```
Laptop (remote agent)                EnterOS Platform
  │                                        │
  │  POST /workspaces                      │
