...
* **Zero-config OSS onboarding** — `git clone enter-os-core && go run ./workspace-server/cmd/server` boots end-to-end without any registry credentials.
...
3. **Embed Dockerfiles in enter-os-core itself, drop the standalone template repos** — would work but breaks the OSS-shape principle; templates are intentionally separable, anyone-can-fork artifacts.
