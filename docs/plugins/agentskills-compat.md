# EnterOS plugins and the agentskills.io standard

> **TL;DR** — every skill inside a EnterOS plugin is a spec-compliant
...
> products that ship the standard today. EnterOS adds a *plugin*
...
```
plugins/my-plugin/                        ← EnterOS bundle (our layer)
├── plugin.yaml                           ← EnterOS manifest: name, version,
│                                            runtimes, adapters, description
├── rules/*.md                            ← EnterOS-only: always-on prose
...
│   └── <skill-name-2>/
│       └── SKILL.md
└── adapters/                             ← EnterOS-only: per-runtime installers
    ├── claude_code.py
    └── deepagents.py
```
...
- **Everything under `skills/<name>/`** follows the spec. A skill-aware
  tool that doesn't know what EnterOS is can consume it as-is.
...
| Spec requirement | How EnterOS enforces it |
|---|---|
...
Rules are a EnterOS-specific extension. If we ever need to represent a
rule as a spec-compliant skill (e.g. for distribution to a non-EnterOS
...
The spec leaves install semantics to the host tool. EnterOS's plugin
...
Any `skills/<name>/` directory from a EnterOS plugin is a valid standalone
...
The skill will use default activation semantics in each tool. EnterOS's
plugin bundle (runtimes, adapters, rules) is not needed — it only matters
if the skill is installed inside EnterOS.
...
- **Zero-cost distribution.** Every skill we ship to EnterOS users is
...
- **Our moat stays intact.** Multi-agent orchestration, A2A, per-runtime
  ...
  spec and is unlikely to be. That's where EnterOS differentiates.
