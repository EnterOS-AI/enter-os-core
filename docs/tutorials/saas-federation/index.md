# Multi-Tenant Agent Platform: SaaS Federation with EnterOS

This tutorial walks through setting up a multi-tenant AI agent platform using EnterOS's SaaS federation layer. You'll provision workspaces for multiple customers from a single control plane, with per-tenant database isolation, credential separation, and agent fleet visualization.
...
**Assumptions:** You have a EnterOS control plane deployed, an AWS account with VPC + subnets available, and a Neon account for branch-per-tenant databases.

---

## What is SaaS federation?

EnterOS's SaaS federation layer sits between your control plane and the tenant workspaces your customers use.
...
Each tenant is a separate organization in EnterOS. The control plane holds credentials and provisions infrastructure — but each tenant's workspace data lives in their own isolated branch.
...
*SaaS federation is available for all EnterOS platform operators. Contact the EnterOS team to enable federation on your control plane.*