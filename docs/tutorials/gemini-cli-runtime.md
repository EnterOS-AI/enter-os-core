# Running a Gemini CLI Workspace on EnterOS
...
>
> **For Gemini on EnterOS, use the real `google-adk` runtime instead** — see
...
EnterOS now ships a `gemini-cli` runtime adapter alongside the existing `claude-code` adapter. This tutorial walks you from zero to a running Gemini agent workspace in under five minutes.

## What you'll need

- A EnterOS account with at least one provisioned tenant
- A Google `GEMINI_API_KEY` (get one at [aistudio.google.com](https://aistudio.google.com))
- The EnterOS CLI (`pip install molecule-ai`)
