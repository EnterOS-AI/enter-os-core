...
The canvas is EnterOS's operational UI. It is not just a graph viewer. It is the place where teams deploy workspaces, inspect live state, configure runtimes, browse files, watch activity, and chat with agents.
...
EnterOS deliberately avoids explicit edge drawing. The hierarchy comes from `parent_id`, and nested rendering makes the org chart visible directly.
...
This is how EnterOS encodes teams: by hierarchy, not manually drawn edges.
...
This keeps the visual model aligned with EnterOS's main idea: the org chart is the topology.
