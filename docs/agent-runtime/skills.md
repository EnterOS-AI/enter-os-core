...
ClawHub skills are context-only (no `tools/` folder). They work in EnterOS as pure context skills — the `SKILL.md` instructions get appended to the agent's system prompt.

### Reusing ClawHub-Style Skills

ClawHub-style skills can be reused here as pure context skills. The `tools/` folder and its MCP tools are included as supporting files when present. Note that `tools/` only execute inside the EnterOS runtime — outside this runtime, the same `SKILL.md` instructions are still useful, but execution remains local.
