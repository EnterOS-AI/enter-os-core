...
- No multi-messenger gateway integration — Hermes's Telegram/Discord/Slack channels are separate from EnterOS's `/channels` feature; map these later via the channels adapter.
- No FTS5 memory backend — use EnterOS's existing `commit_memory` / `search_memory` built-in tools for v1; Hermes-native memory can be layered in a subsequent PR.
