...
description: "EnterOS supports fly.io agent deployment and control-plane provisioning. Switch backends with one env var — no agent code changes required."
tags: [platform, fly.io, deployment, infrastructure]
---

# Deploy AI Agents on Fly.io — or Any Cloud — with One Config Change

Your infrastructure choice just got decoupled from your agent platform choice. EnterOS now ships three production-ready workspace backends — `docker`, `flyio`, and `controlplane` — and switching between them takes a single environment variable. Your agent code, model choices, and workspace topology stay exactly the same.

This post covers what shipped in [PR #501](https://github.com/EnterOS-AI/enter-os-core/pull/501) (Fly Machines provisioner) and [PR #503](https://github.com/EnterOS-AI/enter-os-core/pull/503) (control plane provisioner), and which backend fits your situation.

## Before: One Deployment Model for Every Use Case

Until this week, EnterOS workspaces ran on one backend: Docker. That was the right default for self-hosters — no external dependencies, full control, works anywhere a Docker daemon runs. But it left two groups making a compromise they shouldn't have to:
...
You're already on Fly. You have an account, a Fly app, and you're comfortable with Machines. You want EnterOS workspaces to provision as Fly Machines — no separate Docker host, no idle infrastructure, just workspaces that appear when needed and disappear when they don't.
...
The tenant platform never holds a Fly token. It calls the EnterOS control plane at `https://api.moleculesai.app` (overridable via `CP_PROVISION_URL` for staging environments), which holds Fly credentials and orchestrates workspace provisioning centrally.
...
- **Building a SaaS product on EnterOS?** Set `MOLECULE_ORG_ID` at tenant launch. The control plane provisioner activates automatically. No Fly credentials on the tenant, ever.

**Pick your backend. Deploy your agents.**

→ [Quickstart: choose your deployment backend](/docs/quickstart)

**See also:** [Give Your AI Agent a Real Browser: MCP + Chrome DevTools](/blog/browser-automation-ai-agents-mcp) — wire Chrome DevTools Protocol into your EnterOS workspace via MCP for production-grade browser automation.

---

*[PR #501](https://github.com/EnterOS-AI/enter-os-core/pull/501) (Fly Machines provisioner) and [PR #503](https://github.com/EnterOS-AI/enter-os-core/pull/503) (control plane provisioner) are both merged to `main`. EnterOS is open source — contributions welcome.*
