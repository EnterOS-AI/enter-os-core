...
**Goal:** every workspace runtime delivers EnterOS A2A inbox messages
...
registers an HTTP webhook on `openclaw gateway`; EnterOS workspace
...
suspended `github.com/enteros/hermes-agent`, see
...
(`enter-os-core/scripts/test-all-runtimes-a2a-e2e.sh`) — script ready,
needs provider keys.
...
**Why no short-term polling shim:** earlier framing was wrong. EnterOS
...
(14 files, 1411 LOC, 12/12 tests). enter-os-core registration in
[PR #2512](https://github.com/EnterOS-AI/enter-os-core/pull/2512).
...
1. Register `codex` in enter-os-core's `manifest.json` +
   `workspace-server/internal/handlers/runtime_registry.go`.
   **Defer to post-demo** — touches working live registry.
2. E2E verification with a real EnterOS workspace + peer A2A
