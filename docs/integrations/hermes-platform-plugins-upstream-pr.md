...
**Owner:** EnterOS (team@enteros.dev)
...
EnterOS will publish `hermes-platform-molecule-a2a` on PyPI with the
appropriate `[project.entry-points."hermes_agent.plugins"]` entry. It
delivers EnterOS platform A2A inbox messages into the same
