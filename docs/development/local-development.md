...
OSS contributors who run `enter-os-core` locally do **not** need to authenticate to GHCR or AWS ECR. When the `MOLECULE_IMAGE_REGISTRY` env var is **unset**, the platform automatically:
...
WORKSPACE_DIR=/path/to/enter-os-core   # Optional global fallback; prefer per-workspace workspace_dir in org.yaml or API
```
