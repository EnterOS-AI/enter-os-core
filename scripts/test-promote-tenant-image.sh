...
OUT=$(python3 -c "$PYCODE" 'us-east-1' '<AWS_ACCOUNT_ID>')
python3 -c "import sys,json; d=json.loads(sys.stdin.read()); assert 'commands' in d; c=d['commands'][0]; assert 'us-east-1' in c and '<AWS_ACCOUNT_ID>' in c and c.startswith('aws ecr get-login-password')" <<< "$OUT" \
