...
A merge to `enter-os-core/staging` that touches `workspace/**` triggers
...
```bash
gh workflow list -R Molecule-AI/enter-os-core | grep publish-runtime
# expect: status = disabled_manually
```

If a critical fix MUST ship during the freeze window:

1. `gh workflow enable publish-runtime.yml -R Molecule-AI/enter-os-core`
2. Merge the fix
...
5. `gh workflow disable publish-runtime.yml -R Molecule-AI/enter-os-core` to re-freeze
...
gh run list -R Molecule-AI/enter-os-core --workflow=publish-canvas-image.yml --limit 5
...
The next merge to `enter-os-core/staging` (workspace/**) or any
...
```bash
gh pr list -R Molecule-AI/enter-os-core --base staging --state merged \
